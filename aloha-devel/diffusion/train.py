...
from utils import load_data, load_data_10_250
...
def train(args):
    ...
    save_model_dir=f"{args.ckpt_dir}_observation_horizon{args.observation_horizon}_observation_horizon{args.action_horizon}"
    # fixed parameters
    ...
    wandb.init(project="aloha", 
        config={
            "pred_horizon": args.action_horizon,
            "obs_horizon": args.observation_horizon,
            ...
            "max_episode":args.num_episodes
            
        },
        name=f"demo_aloha_diff_observation_horizon{args.observation_horizon}_observation_horizon{args.action_horizon}",
    )
    # save dataset stats
    if not os.path.isdir(save_model_dir):
        os.makedirs(save_model_dir)
    stats_path = os.path.join(save_model_dir, args.ckpt_stats_name)
    ...
    ckpt_path = os.path.join(save_model_dir, args.ckpt_name)
...
def train_process(train_dataloader, val_dataloader, config, stats,save_model_dir):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if epoch % 100 == 0:
            ckpt_path = os.path.join(save_model_dir, f'policy_epoch_{epoch}_seed_{seed}.ckpt')
