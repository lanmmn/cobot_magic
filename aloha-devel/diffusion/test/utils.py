...
def get_norm_stats(dataset_dir, num_episodes, use_robot_base):
    ...
    # for episode_idx in range(88, 421):
        # if episode_idx == 0:
        #     continue
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            # if action.shape[0] != 300 and action.shape[1] == 14:
...
def load_data_10_250(dataset_dir, num_episodes, arm_delay_time, use_depth_image,
              use_robot_base, camera_names, batch_size_train, batch_size_val):
    print(f'\nData from: {dataset_dir}\n')
    ...
    # construct dataset and dataloader 归一化处理  结构化处理数据
    train_dataset = EpisodicDataset_10_250(train_indices, dataset_dir, camera_names, norm_stats, arm_delay_time,
                                    use_depth_image, use_robot_base)

    val_dataset = EpisodicDataset_10_250(val_indices, dataset_dir, camera_names, norm_stats, arm_delay_time,
                                  use_depth_image, use_robot_base)
